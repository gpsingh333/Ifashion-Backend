...
class AdminLoginView(View):
	...
	def post(self, request, *args, **kwargs):
		...
		if form.is_valid():
			...
			role = UserOtherInfo.objects.select_related('role').values_list('role',flat=True).get(user=user)
...
class EditNewUserView(View):
	...
	def post(self, request, *args, **kwargs):
		...
		form = AdminMemberEditForm(data=request.POST or None, user_id = profile_id)
...
class MemberDateFilterView(View):
	def get(self, request, *args, **kwargs):
		...
		context = {}
		filterdata  = UserOtherInfo.objects.filter(role= role, created__range=(start_date, end_date)).select_related('user').order_by('user__first_name')	
...
class AppInProfileCreateView(View):
	...
	def get(self,request,*args, **kwargs):
		...
		try:
			achievement  = Achievement.objects.filter(designerprofile__user = user).select_related('designerprofile').first()
	...
	def post(self,request,*args, **kwargs):
		...
		if form.is_valid():
			...
			obj = Profile.objects.create(user = request.user,role = role, name =form.cleaned_data.get('name'), email = form.cleaned_data.get('email'),
					profileimg = request.FILES.get('profileimg'), coverimg = request.FILES.get('coverimg'))
...
class AppInProfileEditView(View):
	...
	def post(self,request,*args, **kwargs):
		...
		achievement  = Achievement.objects.filter(designerprofile__user = request.user).select_related('designerprofile').first()
		userdata = achievement.designerprofile
		achievement_img = Image.objects.filter(achievement=achievement)
...
class StylistDesignerCollectionView(View):
	...
	def get(self,request,*args,**kwargs):
		...
		for section in sections:
			...
			section.products = Product.objects.filter(usersection = section, is_deleted=False).select_related('usercategory')		
	...
	def post(self,request,*args,**kwargs):
		...
		if form.is_valid():
			...
			if form.cleaned_data.get('new_from')!='' and form.cleaned_data.get('new_to')!='':
				new_from = datetime.datetime.strptime(form.cleaned_data.get('new_from'), '%m/%d/%Y').strftime('%Y-%m-%d')
			...
			prod_obj = Product.objects.create(name =data['name'].capitalize(), category=category,subcategory=subcategory,
				subsubcategory = subsubcategory, user=request.user,user_role = role, usercategory=owncategory,
				description=data['des'].capitalize(),more_info=data['more_info'].capitalize(),pattern=data['pattern'].capitalize(),
				material =data['material'].capitalize(),fit = data['fit'].capitalize(),
				main_img=request.FILES.get('main_img'),new_from = new_from ,new_to = new_to,)
			...
			if section_id !='' and section_id != None:
				...
				return HttpResponseRedirect('/admin/users/add_product_varients/'+str(prod_obj.id)+'?section_id='+section_id)
...
class StylistDesignerAddProductVarientsView(View):
	def get(self,request,*args,**kwargs):
		...
		context = {
			...
			'colours':Colour.objects.all().exclude(id__in = prod_obj.available_colours.filter(is_active=True).values('colour'))
		}
	...
	def post(self,request,*args,**kwargs):
		...
		product_id = self.kwargs.get('product_id')
		print(data)
		print(request.FILES.getlist('product_img'))
		...
		if form.is_valid():
			...
			if prod_obj:
				...
				for i in range(len(data.getlist('qty'))):
					...
					obj = ProductSizeWithQty.objects.create(size=size,initial_qty=qty,available_qty=qty,sort_id=sort_id)
					print(obj,'size obj')
					size_list.append(obj)
				
				prod_col_obj = ProductAvailableColourWithSizeQty.objects.create(colour_id = data['colour'] ,actual_price=data['price'],
					special_price=special_price,offer=offer)	
				...
				if data.get('click')=='2':
					...
					context = {
						'colours':Colour.objects.all().exclude(id__in = prod_obj.available_colours.all().values('colour'))
						}
...
class StylistDesignerProductDatefilter(View):
	def get(self,request,*args,**kwargs):
		...
		category = Category.objects.all()
		products  = Product.objects.filter(user = user, is_deleted=False,created_date__range=(start_date, end_date)).select_related('usercategory')	
		context = {
		'products':products,
		'categories':category
...
class StylistDesignerProductEditView(View):
	...
	def get(self,request,*args,**kwargs):
		...
		product = Product.objects.select_related('category','subcategory','subsubcategory','usercategory').get(pk=product_id)
		print(product)
...
class StylistDesignerProductVarientEditView(View):
	def get(self,request,*args,**kwargs):
		...
		context = {
			...
			'product_id':self.kwargs.get('product_id'),
			'colours':Colour.objects.all().exclude(id__in = prod_obj.available_colours.filter(is_active=True).exclude(id=self.kwargs.get('varient_id')).values('colour')),
	...
	def post(self,request,*args,**kwargs):
		...
		if request.FILES.getlist('product_img')==[] and not ProductImageByColour.objects.filter(product_colour_id = self.kwargs.get('varient_id')).exists():
			form.add_error(None, "Please select some product images")
		...
		if form.is_valid():
			...
			if prod_obj:
				...
				prod_col_obj = ProductAvailableColourWithSizeQty.objects.prefetch_related('size_and_qty').get(id=self.kwargs.get('varient_id'))
				...
				print(size_list,'sssssssssssssss')
				prod_col_obj.size_and_qty.add(*size_list)
				...
				prod_obj.total_quantity = abs(prod_obj.total_quantity - previous_qty['available_qty__sum'] + total_quantity)
		...
		context = {
			'form':form,
			'available_colours':varient,
			'colours':Colour.objects.all().exclude(id__in = prod_obj.available_colours.all().exclude(id=self.kwargs.get('varient_id')).values('colour')),
			'size_list':list(varient.size_and_qty.all().values_list('id',flat=True))
		}
...
class ProductSearchByCatSubAndSubSubView(View):
	...
	def post(self,request,*args,**kwargs):
		...
		if request.POST['category'] and request.POST['subcategory'] and request.POST['subsubcategory'] is not None:
			user = request.user
			category = Category.objects.all()
			products = Product.objects.filter(user=user,category=request.POST['category'],subcategory=request.POST['subcategory'],subsubcategory=request.POST['subsubcategory'],is_deleted=False)
...
class AddCouponView(View):
	...
	def post(self,request,*args,**kwargs):
		...
		if qs.exists():
			form.add_error(None,'Coupon code with name '+ '('+data.get('code')+')'+ ' already exist.Please try with other')
		...
		if form.is_valid():
			...
			valid_from = datetime.datetime.strptime(form.cleaned_data.get('valid_from'), '%m/%d/%Y').strftime('%Y-%m-%d')
			...
			obj = CouponCode.objects.create(code=request.POST['code'].upper(),coupon_type=request.POST['coupon_type'],
				...
				description=request.POST['description'],is_for_all_user=request.POST['is_all_user'],
...
class EditCouponView(View):
	...
	def get(self,request,*args,**kwargs):
		coupon = CouponCode.objects.prefetch_related('selected_users','selected_product').get(id = self.kwargs.get('coupon_id'))
		...
		products = Product.objects.filter(is_deleted=False).values('name','id','active','created_date').exclude(id__in= coupon.selected_product.all())
		...
		if form.is_valid():
			if data['id'] == 'None':
				...
				ContactAboutTerms.objects.create(about_us=form.cleaned_data['about_us'],
					terms=form.cleaned_data['terms'],contact_us=form.cleaned_data['contact_us'])
				messages.success(request, 'Created successfully')
...
class AddAudiVideoChargesView(View):
	def get(self, request):
		...
		users = UserOtherInfo.objects.filter(role__role_id__in=["3", "4"]).filter(~Q(id__in = charges.values_list('user__id'))).select_related('user')
		return render(request, 'admin-interface/audio_video_charges_management/add_charges.html', context={'charges':charges,'users':users})
	...
	def post(self, request):
		...
		if form.is_valid():
			...
			user_objs = [AudioVideoCharges(audio_charge=audio_charge,video_charge=video_charge, user_id=user) for user in users_list]
...
class OrderView(View):
	...
	def get(self,request,*args,**kwargs):
		...
		if user.is_authenticated:
			...
			if role=='2':
				if startdate is not None and enddate is not None:
					orders = OrderedProductStatus.objects.filter(created__range=(start_date, end_date)).order_by('-created')
				else:
					orders = OrderedProductStatus.objects.all().order_by('-created')
			else:
				if startdate is not None and enddate is not None:
					orders = OrderedProductStatus.objects.filter( created__range=(start_date, end_date) ,cart__product__user = user).select_related('order','user').order_by('-created')
				else:
					orders = OrderedProductStatus.objects.filter(cart__product__user = user).select_related('order','user').order_by('-created')

			return render(request, 'admin-interface/order_management/order_list.html', context={'orders':orders,'status_id':'0','startdate':startdate,'enddate':enddate})
		return render(request, 'admin-interface/order_management/order_list.html', context={'orders':[],'status_id':'0','startdate':startdate,'enddate':enddate})
	...
	def post(self,request,*args,**kwargs):
		...
		if user.is_authenticated:
			...
			if role=='2':
				if status_id=='0':
					orders = OrderedProductStatus.objects.all().select_related('order','user','order__address','cart__product').order_by('-created')
				else:
					orders = OrderedProductStatus.objects.filter(order_status=status_id).select_related('order','user','order__address','cart__product').order_by('-created')
			else:
				if status_id=='0':
					orders = OrderedProductStatus.objects.filter(cart__product__user = user).select_related('order','user','order__address','cart__product').order_by('-created')
				else:
					orders = OrderedProductStatus.objects.filter(order_status=status_id , cart__product__user = user).select_related('order','user','order__address','cart__product').order_by('-created')
	...
	def get(self, request, *args, **kwargs):
		...
		try:
			...
			if order_obj.order_status !='1':
				order_status_detail = OrderStatusChangeDate.objects.filter(order=order_obj, order_status=order_obj.order_status).first()
...
class AddTrackingIdView(View):
	...
	def post(self, request, *args, **kwargs):
		...
		try:
			...
			messages.success(request, 'Status changed and detail sent to user by mail successfully')
...
class PaymentListMemberTransView(View):
	def get(self, request, *args, **kwargs):
		...
		if user.is_authenticated:
			...
			if role == '2':
				if startdate is not None and enddate is not None:
					orders = OrderedProductStatus.objects.filter(order_status='4', created__range=(start_date, end_date)).order_by('-created')
					for order in orders:
						order.payment = MemberPaymentHistory.objects.filter(sub_order_id=order).first()
				else:
					orders = OrderedProductStatus.objects.filter(order_status='4').select_related('order','user').order_by('-created')
					for order in orders:
						order.payment = MemberPaymentHistory.objects.filter(sub_order_id=order).first()
			
				return render(request, 'admin-interface/payment_management/member_transaction_list.html', context={'orders':orders,'sort_id':'1','startdate':startdate,'enddate':enddate})
			else:
				if startdate is not None and enddate is not None:
					orders = OrderedProductStatus.objects.filter(order_status='4', cart__product__user = request.user, created__range=(start_date, end_date)).order_by('-created')
				
					for order in orders:
						order.payment = MemberPaymentHistory.objects.filter(sub_order_id=order).first()
				else:
					orders = OrderedProductStatus.objects.filter(order_status='4',cart__product__user = request.user,).select_related('order','user').order_by('-created')
					for order in orders:
						order.payment = MemberPaymentHistory.objects.filter(sub_order_id=order).first()

				return render(request, 'admin-interface/payment_management/subadmin_member_transaction_list.html', context={'orders':orders,'sort_id':'1','startdate':startdate,'enddate':enddate})
		return render(request, 'admin-interface/payment_management/member_transaction_list.html', context={'orders':orders,'sort_id':'1','startdate':startdate,'enddate':enddate})
		...
		if user.is_authenticated:
			...
			if role == '2':
				...
				if sort_id=='1':
					...
					orders = OrderedProductStatus.objects.filter(order_status='4', order__cart__product__user_role = sort_id).order_by('-created')


				for order in orders:
					order.payment = MemberPaymentHistory.objects.filter(sub_order_id=order).first()
						
				return render(request, 'admin-interface/payment_management/member_transaction_list.html', context={'orders':orders,'sort_id':sort_id})
			else:
				orders = OrderedProductStatus.objects.filter(order_status='4',created__range=(start_date, end_date)).order_by('-created')
				...
				return render(request, 'admin-interface/payment_management/subadmin_member_transaction_list.html', context={'orders':orders,'sort_id':sort_id})
		return render(request, 'admin-interface/payment_management/member_transaction_list.html', context={'orders':orders,'sort_id':sort_id})
...
class PaymentCustomerTransListView(View):
	def get(self, request, *args, **kwargs):
		...
		if user.is_authenticated:
			...
			return render(request, 'admin-interface/payment_management/customer_transaction_list.html', context={'orders':orders,'sort_id':'1','startdate':startdate,'enddate':enddate})
		return render(request, 'admin-interface/payment_management/customer_transaction_list.html', context={'orders':orders,'sort_id':'1','startdate':startdate,'enddate':enddate})
	...
	def get(self, request):
		...
		if user.is_authenticated:
			...
			if role == '2':
				...
				return render(request, 'admin-interface/report_management/report.html',context={'orders':orders, 'total_profit':total_profit, 'sort_id':'1','startdate':startdate,'enddate':enddate})
			else:
				if startdate is not None and enddate is not None:
					orders = OrderedProductStatus.objects.filter(order_status='4', cart__product__user = request.user, created__range=(start_date, end_date)).order_by('-created')
					total_profit = 0
			...
			return render(request, 'admin-interface/report_management/report.html',context={'orders':orders,'sort_id':'1','total_profit':total_profit,'startdate':startdate,'enddate':enddate})


		return render(request, 'admin-interface/report_management/report.html',context={'orders':orders,'sort_id':'1','startdate':startdate,'enddate':enddate})
	...
	def post(self, request, *args, **kwargs):
		...
		if user.is_authenticated:
			...
			if role == '2':
				...
				return render(request, 'admin-interface/report_management/report.html', context={'orders':orders,'total_profit':total_profit,'sort_id':sort_id})
			else:

				orders = OrderedProductStatus.objects.filter(order_status='4',created__range=(start_date, end_date),cart__product__user = request.user).order_by('-created')
				total_profit = 0
				for order in orders:
					special_price = order.cart.selected_colour.special_price
				...
				return render(request, 'admin-interface/report_management/report.html', context={'orders':orders,'total_profit':total_profit,'sort_id':sort_id})
		return render(request, 'admin-interface/report_management/report.html', context={'orders':orders,'sort_id':sort_id})
